...
port_to_pw = pd.read_csv('/net/home.isilon/ag-graw/Software/fitmulticell_slurm_bq/port_pw.csv',
                         squeeze=True,
                         index_col=0,
                         header=None).to_dict()
...
model = fmc.model.MorpheusModel(model_file =  file_, 
                                sumstat_funs = [],
                                executable="/home/bq_nbundgaard/Software/morpheus/build_v2.2.0-beta2/morpheus/morpheus",
                                par_map=par_map,
                                show_stdout=False,
                                show_stderr=False,
...
redis_sampler = RedisEvalParallelSampler(host=args.ip, port=args.port, password=password, look_ahead=True, look_ahead_delay_evaluation=True, log_file='look_ahead.csv')
...
eps = pyabc.ListEpsilon([65, 35.6, 27.2, 20, 16.2, 13.4, 11.4, 9.68, 8.45, 7.46, 6.8, 6.26, 6.15, 5.48, 5.3, 5])  # Defines which distances is needed in next populatio
