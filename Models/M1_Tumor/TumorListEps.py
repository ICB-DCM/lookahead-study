...
# Static Epsilon definition
eps_list = [25000000,24000000,23500000,23000000,22000000]
eps_decrease = 0.8
while eps_list[-1] > 900:
        eps_list.append(eps_decrease*eps_list[-1])

eps = pyabc.ListEpsilon(eps_list)
max_nr_pop = len(eps_list)

# paths of the output-files
basepath="/p/project/fitmulticell/felipe/scripts/Batch_pyABC/programs/Tumor/results"
logfilepath = os.path.join(basepath, "TumorStats"+str(pop_size)+".csv")
resultfile = open(basepath+"/TumorRuntimes"+str(nnodes)+".txt", "a")

db_path = "sqlite:///" + os.path.join(basepath+"/database", "TumorRes"+str(pop_size)+".db")

db_path_ori = "sqlite:///" + os.path.join(basepath+"/database", "TumorRes_ori"+str(pop_size)+".db")
...
plt.savefig(os.path.join(basepath,"TumorResPPP"+str(pop_size)+".pdf"))
...
pyabc.visualization.plot_kde_matrix(df, w, limits=limits);
plt.savefig(os.path.join(basepath, "TumorResOri"+str(pop_size)+".pdf"))
