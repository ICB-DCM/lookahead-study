...
do 
	...
	sbatch --account=fitmulticell --time=${TIME} --job-name=${NAME}  --dependency=after:$JOB_ID --cpus-per-task=${CPUSPERTASK} --partition=${PARTITION}  submit_nodaemonworker.sh ${REDIS_IP} ${PORT} ${TIME} ${CPUSPERTASK} ${PWD}
done
...
sbatch --account=fitmulticell --time=${TIME} --job-name=python_${PORT} --dependency=after:$JOB_ID --partition=${PARTITION} submit_python.sh ${PWD} ${REDIS_IP} ${PORT} ${N_NODES} ${PYTHONFILE}
